...
class ConversationTests(test.base.BaseAsyncTestCase):
    ...
    def test_muting_conversation(self):
        ...
        response = self.fetch('/conversations/%s/mute' % admin_conversation.id, method='POST', headers={'Cookie':'_xsrf=%s;sid=%s' % (self.xsrf, self.sid)}, body="_xsrf=%s" % self.xsrf)
        response = self.fetch('/conversations/%s/mute' % user2_conversation.id, method='POST', headers={'Cookie':'_xsrf=%s;sid=%s' % (self.xsrf, self.sid)}, body="_xsrf=%s" % self.xsrf)
